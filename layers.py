...
class DenseModel_B(nn.Module):
    """Stack of fully connected layers."""
    def __init__(self, num_hidden, out_dim, num_layers, dropout, device):
        super(DenseModel_B, self).__init__()
    ...
    def forward(self, x):
        x = self.dropout_layer(x)
        x = nn.Linear(x.shape[-1],self.num_hidden).to(self.device)(x)
        x = nn.Linear(self.num_hidden,self.out_dim).to(self.device)(x)
        x = torch.tanh(x)
        x = self.dropout_layer(x)
        x = nn.Linear(self.num_hidden,self.out_dim).to(self.device)(x)
